...
export default function RowPaymentProducts({ order }) {
    ...
    return (
        <div className="col lastPayment">
            ...
            <div className="row">
                <div className="col">
                    <h3>Entrega estimada: {`${today.getDate() + 3} ${today.toLocaleString('default', { month: 'short' })} ${today.getFullYear()}`}</h3>
                    {order.products.length > 0 && order.products.map(product =>
                        <div className="row" key={product._id}>
                            {product.image === null
                                ? <img src="https://ia-latam.com/wp-content/uploads/2018/12/No-image-found-1.jpg" alt={`portada ${product.name}`} />
