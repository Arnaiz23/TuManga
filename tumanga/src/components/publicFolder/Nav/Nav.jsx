...
export default function Nav({ state, user, changeProcess }) {
  ...
  return (
    <nav id="navHome" className={navState} ref={navRef}>
      <i className="closeNav" onClick={closeNav}>
      ...
      <div className="navOptions">
        <ul>
          ...
          <li>
            <Link to="/products/mangas">
              <i className="fa-solid fa-book"></i>Mangas
            </Link>
          </li>
          <li>
            <Link to="/products/merchandising">
              <i className="fa-solid fa-child-reaching"></i>Merchandising
            </Link>
          </li>
          <li>
            {tokenInfo && (
      ...
      {ROLES.includes(userData.roleName) && (
        <Link to="/platform">
          <button className="btn btn-light" id="btnAccessPanel">
            Acceder al panel
          </button>
        </Link>
      )}
      {tokenInfo ? (
        <button className="btn btn-danger" id="btnLogout" onClick={logout}>
          Cerrar sesión
        </button>
      ) : (
        <Link to="/login">
