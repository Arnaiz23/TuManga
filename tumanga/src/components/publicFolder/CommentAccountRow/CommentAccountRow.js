        ...
        <div className="comment">
            <header className="row">
                <div className="rowStars">
                    <div className="rowStars">
                        {stars.map(i => i <= data.score
                            ? <i key={`${i}${data._id}`}><FontAwesomeIcon icon={faStar} className="starsActive" /></i>
                            : <i key={`${i}${data._id}`}><FontAwesomeIcon icon={faStar} /></i>
                        )}
                    </div>
                </div>
                <h3>{data.product_name}</h3>
                <i className="deleteComment" onClick={deleteComment}><FontAwesomeIcon icon={faTrash} /></i>
            </header>
            <main className="row">
                <p>{data.message}</p>
